...
# 1. CONFIG & THEME
# ==========================================
st.set_page_config(page_title="YouTube Command Center A.X.G", page_icon="💎", layout="wide")
...
st.markdown("""
    ...
    .hud-card:hover { border-color: #CC0000; transform: translateY(-2px); }
    .hud-value { font-size: 24px; font-weight: 700; color: #FFFFFF; font-family: 'Courier New', monospace; }
    .hud-label { font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 2px; }
    
    /* Custom Branding */
    .brand-text { color: #FF0000; font-weight: bold; letter-spacing: 1px; }
...
def ai_forensic_audit(transcript, title, duration):
    ...
    * **Est. Cuts Per Minute:** (Based on word count)
...
query = st.text_input("TARGET VECTOR", placeholder="e.g. 'MrBeast'", label_visibility="collapsed")
...
if st.session_state.search_done:
    ...
    with tabs[0]:
        st.dataframe(
            df[['Thumbnail', 'Title', 'Views', 'Duration', 'Virality Score']], 
    ...
    with tabs[2]:
        ...
        with c2:
            if st.session_state.all_tags:
                wc = WordCloud(width=800, height=400, background_color='#0f0f0f', colormap='Greens').generate_from_frequencies(dict(tag_counts))
                fig, ax = plt.subplots(); plt.imshow(wc, interpolation='bilinear'); plt.axis("off"); fig.patch.set_facecolor('#0f0f0f'); st.pyplot(fig)
    
    # TAB 4: DEEP DIVE (Legacy)
    with tabs[3]:
        st.video(row['Link'])
